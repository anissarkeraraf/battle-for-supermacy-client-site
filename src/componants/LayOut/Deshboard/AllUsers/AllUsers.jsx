...
const AllUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axiosSecure.get(`/donor?status=${filter}&page=${page}&perPage=${perPage}`);
  ...
  const handleBlock = async (userId) => {
    ...
    await axiosSecure.post(`/users/${userId}/block`);
    fetchUsers();
  };

  const handleUnblock = async (userId) => {
    // Implement unblock user logic
    await axiosSecure.post(`/users/${userId}/unblock`);
    fetchUsers();
  };

  const handleMakeVolunteer = async (userId) => {
    // Implement make volunteer logic
    await axiosSecure.post(`/users/${userId}/volunteer`);
    fetchUsers();
  };
    ...
    .then(res => {
        ...
        if (res.data.modifiedCount > 0) {
            ...
            Swal.fire({
                position: "center",
                icon: "success",
                title: `${donor.name} is an admin now`,
